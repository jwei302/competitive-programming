...
//the range is from to search from is 2000 by 2000, which is small enough for us to check each coordinate.

vii coords(3, vi(4));

bool ok(int x, int y){
	return ((coords[0][0]<=x&&x<coords[0][2]&&coords[0][1]<=y&&y<coords[0][3])||(coords[1][0]<=x&&x<coords[1][2]&&coords[1][1]<=y&&y<coords[1][3]))&&!(coords[2][0]<=x&&x<coords[2][2]&&coords[2][1]<=y&&y<coords[2][3]);
}
int main(){
	setIO("billboard");
	read(coords);
