...
//Brute force, try putting the stone under each shell and simulating and find the maximum amount of correct guesses.
...
int main(){
	...
	FOR(stone, 3){
		...
		shells[stone] = 1;
		FOR(i, n){
			swap(shells[round[i][0]-1], shells[round[i][1]-1]);
			if(shells[round[i][2]-1])
				cur++;
		}
		fill(all(shells), 0);
		ans = max(cur, ans);
