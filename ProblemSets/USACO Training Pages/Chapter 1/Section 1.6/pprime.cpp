...
ll creatingPalindrome(ll input, ll isOdd){
...
vl generatePalindromes(ll a, ll b){
	vl palindromes;
	ll number;
	FOR(j, 2){
		ll i = 1;
		while((number = creatingPalindrome(i, j%2))<=b){
			i++;
			if(number<a)
				continue;
			if(isPrime(number))
				palindromes.pb(number);
		}
	}
	return palindromes;
}
int main(){
	...
	ll a, b;
	read(a, b);
	vl palindromes = generatePalindromes(a, b);
	sort(all(palindromes));
	FOR(i, sz(palindromes))
		prints(palindromes[i]);
	return 0;
}
