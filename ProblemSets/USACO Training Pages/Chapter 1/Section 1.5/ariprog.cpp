...
int main(){
	...
	FOR(p, m+1){
		FOR(q, m+1){
			if(!vis[p*p+q*q])
				bisquares.pb(p*p+q*q);
			vis[bisquares.back()] = 1;
		}
	}
	sort(all(bisquares));
	vpi ans;
	FOR(i, sz(bisquares)){
		F0R(j, i+1, sz(bisquares)){
			bool ok = 1;
			int first = bisquares[i], diff = bisquares[j]-bisquares[i];
			if(first+(n-1)*diff>bisquares.back())
				break;
			F0R(k, 1, n){
				if(!vis[first + k*diff]){
					ok = 0;
