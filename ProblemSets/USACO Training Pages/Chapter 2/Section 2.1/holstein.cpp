...
const int mxV = 25, mxG = 15;
int v, g;
vi vitamins(mxV+5);
vii amt(mxG+5, vi(mxV+5));

int main(){
	setIO("holstein");
	read(v);
	FOR(i, v) read(vitamins[i]);
	read(g);
	FOR(i, g) FOR(j, v) read(amt[i][j]);
	vii subsets;
	FOR(i, (1<<g)){
		vi subset;
		FOR(j, g) if(i&(1<<j)) subset.pb(j);
		if(sz(subset)) subsets.pb(subset);
	}
	vi cur(v), ans(g), poss(g);
	int min_size = INT_MAX;
	FOR(i, sz(subsets)){
		FOR(j, v) cur[j] = 0;
		FOR(j, sz(subsets[i])) {
			poss[j] = subsets[i][j]+1;
			FOR(k, v) cur[k]+=amt[subsets[i][j]][k];
		}
		bool ok = 1;
		FOR(j, v) if(vitamins[j]>cur[j]) ok = 0;
		if(ok){
			if(sz(subsets[i])<min_size){
				min_size = sz(subsets[i]);
				ans = poss;
			}
			if(sz(subsets[i])==min_size) ans = min(poss, ans);
		}
	}
	print(min_size);
	FOR(i, min_size) print(" ", ans[i]);
	prints();
	return 0;
}
