...
int n;
map<int, str, greater<int>> numerals{{1000, "M"},{4, "IV"}, {9, "IX"}, {40, "XL"}, {90, "XC"}, {400, "CD"}, {900, "CM"}, {500, "D"}, {100, "C"}, {50, "L"}, {10, "X"}, {5, "V"}, {1, "I"}};

str convert(int n){
	str res = "";
	trav(key, numerals){
		if(n<key.f) continue;
		FOR(i, n/key.f) res+=key.s;
		n %= key.f;
	}
	return res;
}
int main(){
	...
	vc numeral{'I', 'V', 'X', 'L', 'C', 'D', 'M'};
	FOR(i, sz(numeral)) if(freq[numeral[i]]) prints(numeral[i], freq[numeral[i]]);
	return 0;
}
