...
//brute force search all the possible triangles coordinates
	...
	FOR(i, n){
		FOR(j, n){
			...
			FOR(k, n){
				if(i==k||j==k)
					continue;
				if(coords[i].f==coords[j].f&&coords[j].s==coords[k].s){
					ans = max(abs(coords[i].s-coords[j].s)*abs(coords[j].f-coords[k].f), ans);
				}
			}
		}
	}
