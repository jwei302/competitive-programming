...
int main(){
	...
	int a, b, x, y;
	read(a, b, x, y);
	int without_teleporter_dist = abs(a-b), with_teleporter_dist, starting_with_x, starting_with_y;
	starting_with_x = abs(a-x)+abs(b-y);
	starting_with_y = abs(a-y)+abs(b-x);
	with_teleporter_dist = min(starting_with_x, starting_with_y);
	prints(min(without_teleporter_dist, with_teleporter_dist));
	return 0;
}
